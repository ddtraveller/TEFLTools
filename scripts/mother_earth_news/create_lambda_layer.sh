...
create_layer() {
    ...
    # Cleanup memory before publishing
    cleanup_memory
...
mkdir -p "${NLP_LAYER_DIR}/nltk_data"

# Install NLTK, Stanza, and tokenizers in the NLP layer
pip install nltk==3.8.1 stanza==1.4.2 tokenizers==0.13.2 --target "${NLP_LAYER_DIR}/python" --no-cache-dir

# Download NLTK data
PYTHONPATH="${NLP_LAYER_DIR}/python" python -c "
import nltk
nltk.download('punkt', download_dir='${NLP_LAYER_DIR}/nltk_data')
nltk.download('stopwords', download_dir='${NLP_LAYER_DIR}/nltk_data')
"
...
import os
import stanza
os.environ['STANZA_RESOURCES_DIR'] = '${NLP_LAYER_DIR}/python/stanza_resources'
stanza.download('en')
"
...
NLP_LAYER_ARN=$(aws lambda publish-layer-version \
    ...
    --description "Layer for NLP tools including NLTK, Stanza, and tokenizers with minimal data" \
    --license-info "MIT" \
...
echo "Lambda layers created and published successfully."