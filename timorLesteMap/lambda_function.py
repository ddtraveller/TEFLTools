...
def lambda_handler(event, context):
    ...
    lat, lon = message_parts[2].split(',')
    label = ' '.join(message_parts[3:]) if len(message_parts) > 3 else ''
    print(lat, lon, label)
    ...
    # Add existing markers from the CSV to the map
    for marker in markers:
    ...
    print('action ' + action)
    if action == 'add':
        color = color_list[len(markers) - 1]
        ...
        legend_html += f'<p style="margin: 0;"><span style="color: {color}">&#9679;</span> {label}</p>'
    elif action == 'drop':
        # Remove the marker from the CSV and the map if it exists
        markers = [marker for marker in markers if marker[:2] != [lat, lon]]
        for marker in dili_map._children.values():
            if isinstance(marker, folium.Marker):
                if marker.location == [float(lat), float(lon)]:
                    dili_map.remove_layer(marker)
                    break
